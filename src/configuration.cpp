...
//         std::string target = line.substr(0, open);
...
// ✅ FIX: Added helper function for location prefix normalization
std::string normalize_location_prefix(const std::string& prefix) {
    if (prefix.empty()) 
        return "/";
...
// ✅ FIX: Modified to normalize location prefix before storing
bool find_location(std::string line, server_rule &server_1)
{
    while (true)
    {
        ...
        // ✅ FIX: Normalize location prefix BEFORE storing in map
        std::string normalized_target = normalize_location_prefix(target);
        server_1.location_map[normalized_target] = loc;  // ← CRITICAL FIX
