...
// 	{
// 		// std::cout << "Accept a clinet in this port : " << port_[num_socket]<< "\n";
// 		// port_.push_back(servers[read_server].listen_port);
// 		setup_socket();
// 		rebuild_poll_fds();
// 		read_server++;		
// 		// num_socket++;
...
void Server::handle_client_read(int fd)
{
	...
	it->second.in.append(buf, n);
	// if (it->second.in.find("\r\n\r\n") != std::string::npos && it->second.out.empty())
	// {
		it->second.out = build_basic_response();
	// }
}
...
void Server::handle_client_write(int fd)
{
	...
	int n = send(fd, it->second.out.c_str(), it->second.out.size(), 0);
	if (n <= 0)
	{
		close_client(fd);
		return;
	...
	it->second.out.erase(0, static_cast<size_t>(n));
	if (it->second.out.empty())
		close_client(fd);
}
		...
		for (size_t i = 0; i < poll_fds_.size(); ++i)
		{
			...
			if (is_port(poll_fds_[i].fd))
			{
				if ( poll_fds_[i].revents & POLLIN) // this is use a bitwise 
					{
						// std::cout << "Accept a clinet in this port : " << fd_[j] << port_[num_socket]<< "\n";
